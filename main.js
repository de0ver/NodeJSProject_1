...
function postPage(id)
{
    ...
    fetch(`https://gorest.co.in/public/v2/posts/${id}`).then(async (res) => {
        ...
        setChildren(body, el('div', {className: 'card-body'}, [el('h2', data.title), el('p', data.body), el('a', {className: 'btn btn-primary'}, {href: '/', onclick: navigate, }, 'Вернуться к списку'), ]));
    });
...
function postListPage()
{
    ...
    fetch("https://gorest.co.in/public/v2/posts").then(async (res) => {
        const data = await res.json();
        setChildren(list, data.map((post) => el('li', {className: 'list-group-item'}, el('a', {href: `/post/${post.id}`, onclick: navigate,}, post.title))));
    });
